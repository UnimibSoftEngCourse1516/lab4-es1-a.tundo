...
public final class CassandraDataModel implements DataModel, Closeable {
	...
	public Float getPreferenceValue(long userID, long itemID) {
		ColumnQuery<Long, Long, Float> query = HFactory.createColumnQuery(keyspace, LongSerializer.get(),
	...
	public Long getPreferenceTime(long userID, long itemID) {
		ColumnQuery<Long, Long, ?> query = HFactory.createColumnQuery(keyspace, LongSerializer.get(),
	...
	public int getNumItems() {
		...
		if (itemCount == null) {
			CountQuery<Long, Long> countQuery = HFactory.createCountQuery(keyspace, LongSerializer.get(),
					LongSerializer.get());
	...
	public int getNumUsersWithPreferenceFor(long itemID) throws TasteException {
		 ...
		 * query.setColumnFamily(ITEMS_CF); query.setKey(itemID);
		 * query.setRange(null, null, Integer.MAX_VALUE); return
		 * query.execute().get();
		 */
	...
	public int getNumUsersWithPreferenceFor(long itemID1, long itemID2) throws TasteException {
		...
		return userIDs1.size() < userIDs2.size() ? userIDs2.intersectionSize(userIDs1)
				: userIDs1.intersectionSize(userIDs2);
	}

	/*
	 * La violazione segnalata era <<Method parameters, caught exceptions and
	 * foreach variables should not be reassigned>>. Ho corretto introducendo una
	 * nuova variabile secureValue come suggerito assegnadole il valore di value
	 * se esso è un numero altrimenti 1.0f come già fatto prima della mia
	 * correzione. A livello di leggibilità, secondo me in questo caso si poteva
	 * anche a lasciare così com'era. Era molto più intuitivo capire dal codice
	 * originale che se value non è un numero, gli veniva assegnato 1.0f,
	 * altrimenti si utilizzava direttamente il valore passato. Temo che la mia
	 * correzione abbia appesantito la leggibilità e la complessità.
	 */
	...
	public void setPreference(long userID, long itemID, float value) {
		...
		itemForUsers.setValue(secureValue);
		...
		userForItems.setValue(secureValue);
	...
	private SliceQuery<Long, Long, byte[]> buildNoValueSliceQuery(String cf) {
		SliceQuery<Long, Long, byte[]> query = HFactory.createSliceQuery(keyspace, LongSerializer.get(),
				LongSerializer.get(), BytesArraySerializer.get());
	...
	private SliceQuery<Long, Long, Float> buildValueSliceQuery(String cf) {
		SliceQuery<Long, Long, Float> query = HFactory.createSliceQuery(keyspace, LongSerializer.get(),
				LongSerializer.get(), FloatSerializer.get());
		query.setColumnFamily(cf);
		query.setRange(null, null, false, Integer.MAX_VALUE);
		return query;
