...
public final class OrderedIntDoubleMapping implements Serializable, Cloneable {
	 ...
	 * through the existing indices and values and the updates' indices and
	 * values at the same time while selecting the minimum index to set at each
	 * step.
	 * 
	 * @param updates
	...
	public void merge(OrderedIntDoubleMapping updates) {
		...
		int i = 0, j = 0;
		/*
		 * SonarQube segnalava la violazione <<"for" loop incrementers should
		 * modify the variable being tested in the loop's stop condition>>
		 * Effettivamente nel for qui di seguito la condizione è su i e j mentre
		 * invece viene incrementata la variabile k. I tre if contenuti nel
		 * ciclo servono a determinare se devo far avanzare i o j per riuscire a
		 * fare il merge correttamente come descritto nel commento iniziale del
		 * metodo, mentre la variabile k (indice dei nuovi array che sto
		 * costruendo) deve avanzare ad ogni iterazione a prescindere da come
		 * determino i nuovi indici e i nuovi valori, e quindi a prescindere
		 * dall'incremento di i o j. Devo però terminare il for quando non ho
		 * più 'indices' o 'updateIndeces', quindi, la condizione deve essere su
		 * i e j, mentre k deve essere incrementata ad ogni iterazione. Una
		 * possibile soluzione è quella di spostare l'incremento di k o
		 * all'interno di ogni if presente nel for, oppure immediatamente dopo
		 * in modo che comunque venga eseguito, potrebbe risultare meno compatto
		 * ma più capibile e forse non violerebbe le rules di SonarQube. O
		 * ancora si potrebbe riscrivere il tutto con un while.
		 * 
		 * Concludendo, io credo che sia un falso positivo, anche se è
		 * concettualmente sbagliato, in questo specifico caso ha senso scrivere
		 * un for in questo modo per il tipo di considerazioni fatte sopra.
		 */
